...
from the same date the mean is calculated. Note therefore that index labvalues can be after index. 
...
Note that there are modifications to selection of variables in model to what is stated in the protocol. 
...
Note that a cut-off of 3000 is used instead of 5000 (so stable low is defined as <3000). Further defined in 3 groups (Decrease > 20%, Stable (within +/-20%), Increase > 20%). 

The 6 mo pre and post index are restricted to be within 6 months +/- 3 mo. The labvalues closest to 3 and 6 months respectively are selected. 
...
Since the index measurement can be AFTER WHFE is this a problem? 
...
Note that the variables are collected at index (and not 6 mo after index, although time starts 6 mo after index).
